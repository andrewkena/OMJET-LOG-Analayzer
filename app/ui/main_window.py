...
SPEED_GAUGE_CANDIDATES = [("GPS", "Spd", 1.0), ("VFR_HUD", "groundspeed", 1.0)]
...
class MainWindow(QMainWindow):
    ...
    def _build_layout(self):
        ...
        self.speed_combo.addItems(["x1", "x2", "x4"])
        ...
        bottom_row.addWidget(self.rc_panel)
        bottom_row.addWidget(bottom_separator())
        bottom_row.addWidget(self.motor_servo_panel)
        bottom_row.addWidget(bottom_separator())
        bottom_row.addWidget(self.attitude_panel)
    ...
    def _load_timeline(self):
        ...
        self.timeline_widget.set_range(
            self.log_data.start_time, self.log_data.end_time, backdrop_t, backdrop_y
        )
    ...
    def _load_speed_alt_gauges(self):
        if self.log_data is None:
            return
        for msg_type, field, divisor in SPEED_GAUGE_CANDIDATES:
            table = self.log_data.messages.get(msg_type)
    ...
    def _load_motor_servo_panel(self):
        ...
        for label in SERVO_FUNCTION_IDS:
